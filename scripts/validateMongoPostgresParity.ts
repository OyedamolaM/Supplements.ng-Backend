...
const main = async () => {
  ...
  try {
    ...
    const mongoShippingAddresses = usersRaw.reduce(
      (sum, user) => sum + (Array.isArray(user.shippingAddresses) ? user.shippingAddresses.length : 0),
      0
    );
    const mongoWishlistItems = usersRaw.reduce(
      (sum, user) => sum + (Array.isArray(user.wishlist) ? user.wishlist.length : 0),
      0
    );
    const mongoCartItems = usersRaw.reduce(
      (sum, user) => sum + (Array.isArray(user.cart) ? user.cart.length : 0),
      0
    );
    const mongoOrderItems = ordersRaw.reduce(
      (sum, order) => sum + (Array.isArray(order.products) ? order.products.length : 0),
      0
    );
    const mongoSupplierInvoiceItems = supplierInvoicesRaw.reduce(
      (sum, invoice) => sum + (Array.isArray(invoice.items) ? invoice.items.length : 0),
      0
    );
    const mongoSupplierInvoicePayments = supplierInvoicesRaw.reduce(
      (sum, invoice) => sum + (Array.isArray(invoice.payments) ? invoice.payments.length : 0),
      0
    );
    const mongoSupplierInvoiceAttachments = supplierInvoicesRaw.reduce(
      (sum, invoice) => sum + (Array.isArray(invoice.attachments) ? invoice.attachments.length : 0),
      0
    );
    ...
    const [
      ...
      pgCartItems,
      pgOrderItems,
      pgSupplierInvoiceItems,
      pgSupplierInvoicePayments,
      pgSupplierInvoiceAttachments,
    ] = await Promise.all([
      prisma.branch.count(),
      prisma.user.count(),
    ...
    for (const row of rows) {
      ...
      console.log(
        `${status.padEnd(9)} ${row.key.padEnd(28)} mongo=${String(row.mongo).padEnd(8)} postgres=${row.postgres}`
      );
    }
