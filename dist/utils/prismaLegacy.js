...
const toLegacyProduct = (product) => {
    ...
    return {
        ...
        reorderable: product.reorderable ?? true,
        recommendedDosageAmount: product.recommendedDosageAmount || 0,
        recommendedDosageUnit: product.recommendedDosageUnit || "",
        recommendedFrequencyPerDay: product.recommendedFrequencyPerDay || 0,
        recommendedUsageText: product.recommendedUsageText || "",
        usageMode: product.usageMode ? product.usageMode.toString().toLowerCase() : "fixed",
        allowManualUsageOverride: product.allowManualUsageOverride ?? true,
...
const toLegacyOrder = (order) => {
    ...
    return {
        ...
        products: items.map((item) => ({
            ...
            price: item.price,
            recommendedUsageTextSnapshot: item.recommendedUsageTextSnapshot || "",
            recommendedDosageAmountSnapshot: item.recommendedDosageAmountSnapshot || 0,
            recommendedDosageUnitSnapshot: item.recommendedDosageUnitSnapshot || "",
            recommendedFrequencyPerDaySnapshot: item.recommendedFrequencyPerDaySnapshot || 0,
            usageModeSnapshot: item.usageModeSnapshot || "FIXED",
            dosageRecommendedByName: item.dosageRecommendedByName || null,
            dosageRecommendedByRole: item.dosageRecommendedByRole || null,
            dosageRecommendedAt: item.dosageRecommendedAt || null,
        })),
        shippingAddress: {
...
const legacyOrderStatusToDb = (status) => {
    ...
    if (key === "processing")
        return "PROCESSING";
    if (key === "pending dispatch")
        return "SHIPPED";
    if (key === "shipped")
        return "SHIPPED";
    if (key === "in transit" || key === "in_transit" || key === "intransit")
        return "SHIPPED";
    if (key === "arrived")
        return "SHIPPED";
    if (key === "delivered")
