...
const parseArgs = () => {
    ...
    if (positional.length) {
        if (!map.email && positional[0])
            map.email = positional[0];
        if (!map.password && positional[1])
            map.password = positional[1];
    }
    return map;
};
...
const check = async (results, name, fn) => {
    try {
...
const main = async () => {
    ...
    const adminEmail = pick(args.email, process.env.SMOKE_ADMIN_EMAIL, process.env.BOOTSTRAP_SUPER_ADMIN_EMAIL).toLowerCase();
    const adminPassword = pick(args.password, process.env.SMOKE_ADMIN_PASSWORD, process.env.BOOTSTRAP_SUPER_ADMIN_PASSWORD);
    ...
    if (!adminEmail || !adminPassword) {
        throw new Error("Missing admin credentials. Pass --email and --password or set SMOKE_ADMIN_EMAIL and SMOKE_ADMIN_PASSWORD.");
    }
    const baseUrl = `http://127.0.0.1:${port}`;
    const results = [];
    let server = null;
    if (!useRunning) {
        server = (0, child_process_1.spawn)("node", ["dist/server.js"], {
            cwd: process.cwd(),
    ...
    try {
        ...
        await check(results, "create product admin", async () => {
            const res = await request(baseUrl, "POST", "/api/admin/products", adminToken, {
        ...
        await check(results, "admin list orders", async () => {
            ...
            const hasOrder = Array.isArray(res.data)
                ? res.data.some((order) => (order._id || order.id) === orderId)
                : false;
            return {
                status: res.status,
        ...
        await check(results, "admin update order status", async () => {
            const res = await request(baseUrl, "PUT", `/api/admin/orders/${orderId}`, adminToken, { status: "Shipped" });
            if (!res.ok)
                throw new Error(`Expected 200 got ${res.status}`);
        ...
        await check(results, "reports sales summary", async () => {
            ...
            const res = await request(baseUrl, "GET", `/api/reports/sales-summary?from=${today}&to=${today}&group=day`, adminToken);
            if (!res.ok)
                throw new Error(`Expected 200 got ${res.status}`);
