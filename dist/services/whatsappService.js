...
const ensureWhatsAppConfig = () => {
    if (!ACCESS_TOKEN || !PHONE_NUMBER_ID) {
        const error = new Error("WhatsApp Cloud API is not configured. Set WHATSAPP_CLOUD_API_TOKEN and WHATSAPP_PHONE_NUMBER_ID.");
        error.status = 500;
        throw error;
    }
};
...
const sendWhatsAppTemplate = async ({ to, templateName, components, language = "en_US", }) => {
    ...
    if (!response.ok) {
        const error = new Error("WhatsApp Cloud API error.");
...
const sendWhatsAppOtp = async ({ to, code, minutes, }) => {
    const normalized = normalizeWhatsAppRecipient(to);
    if (!normalized || !isE164(normalized)) {
        const error = new Error("Recipient phone must be in E.164 format.");
        error.status = 400;
        throw error;
    }
...
const sendOrderStatusWhatsApp = async ({ to, orderId, status, }) => {
    const normalized = normalizeWhatsAppRecipient(to);
    if (!normalized || !isE164(normalized)) {
        const error = new Error("Recipient phone must be in E.164 format.");
        error.status = 400;
...
const sendRefillReminderWhatsApp = async ({ to, productName, message, }) => {
    const normalized = normalizeWhatsAppRecipient(to);
    if (!normalized || !isE164(normalized)) {
        const error = new Error("Recipient phone must be in E.164 format.");
        error.status = 400;
