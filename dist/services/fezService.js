...
const parseExpiry = (value) => {
    ...
    if (/^\d+$/.test(raw)) {
        ...
        if (!Number.isFinite(numeric))
            return null;
        return numeric > 2000000000 ? numeric : numeric * 1000;
    }
...
const extractToken = (payload) => payload?.authToken ||
    ...
    payload?.data?.token ||
    null;
const extractExpiry = (payload) => parseExpiry(payload?.expireToken) ||
...
const requestFez = async (path, options = {}) => {
    ...
    if (!response.ok) {
        const error = new Error(typeof data === "string" && data ? data : `Fez request failed (${response.status})`);
...
const resolveTrackPath = (orderNumber) => {
    if (!orderNumber)
        return null;
...
const extractOrderNumber = (payload, uniqueId) => {
    if (!payload)
        return null;
    ...
    if (payload?.orderNo?.[uniqueId])
        return payload.orderNo[uniqueId];
    if (payload?.orderNos?.[uniqueId])
        return payload.orderNos[uniqueId];
    if (payload?.data?.orderNos?.[uniqueId])
        return payload.data.orderNos[uniqueId];
    if (payload?.Response?.[uniqueId])
        return payload.Response[uniqueId];
...
const buildFezOrderPayload = (order, context = {}) => {
    ...
    const senderOverrides = (contextThirdParty && (contextSenderName || contextSenderAddress || contextSenderPhone)) ||
...
exports.createFezOrder = async ({ order, context = {} }) => {
    if (!order?.id)
        throw new Error("Order payload is required for Fez");
...
exports.trackFezOrder = async (orderNumber) => {
...
exports.fetchFezDeliveryCost = async (payload) => {
    return requestFez("/order/cost", {
        method: "POST",
        body: JSON.stringify(payload),
    });
};
exports.createFezOrderFromPayload = async (payload) => {
    if (!payload)
        throw new Error("Fez order payload is required");
    return requestFez("/order", {
        method: "POST",
        body: JSON.stringify(payload),
    });
};
exports.updateFezOrder = async (payload) => {
    if (!payload)
        throw new Error("Fez update payload is required");
    return requestFez("/order", {
        method: "PUT",
        body: JSON.stringify(payload),
    });
};
exports.deleteFezOrder = async (payload) => {
    if (!payload)
        throw new Error("Fez delete payload is required");
    return requestFez("/order", {
        method: "DELETE",
        body: JSON.stringify(payload),
    });
};
exports.getFezOrder = async (orderId) => {
    if (!orderId)
        throw new Error("Order ID is required");
    return requestFez(`/orders/${encodeURIComponent(orderId)}`, { method: "GET" });
};
exports.searchFezOrders = async (payload) => {
    return requestFez("/orders/search", {
        method: "POST",
        body: JSON.stringify(payload || {}),
    });
};
exports.searchFezOrderByWaybill = async (waybillNumber) => {
    if (!waybillNumber)
...
exports.getFezOrderStatsWithDateRange = async (payload) => {
    return requestFez("/orders/statsWithDateRange", {
        method: "POST",
        body: JSON.stringify(payload || {}),
    });
};
exports.getFezDeliveryTimeEstimate = async (payload) => {
    return requestFez("/delivery-time-estimate", {
        method: "POST",
        body: JSON.stringify(payload || {}),
    });
};
exports.getFezPickupHubs = async (stateId) => {
    if (!stateId)
...
exports.getFezLockersByState = async (state) => {
    if (!state)
        throw new Error("State is required");
    return requestFez(`/Lockers/${encodeURIComponent(state)}`, { method: "GET" });
};
exports.createFezImportOrder = async (payload) => {
    if (!payload)
        throw new Error("Fez import payload is required");
    return requestFez("/orders/import", {
        method: "POST",
        body: JSON.stringify(payload),
    });
};
exports.createFezExportOrder = async (payload) => {
    if (!payload)
        throw new Error("Fez export payload is required");
    return requestFez("/orders/export", {
        method: "POST",
        body: JSON.stringify(payload),
    });
};
exports.getFezImportDeliveryCost = async (payload) => {
    return requestFez("/orders/import-price", {
        method: "POST",
        body: JSON.stringify(payload || {}),
    });
};
exports.getFezExportDeliveryCost = async (payload) => {
...
exports.createFezUser = async (payload) => {
    if (!payload)
        throw new Error("Fez user payload is required");
...
exports.registerFezOrderWebhook = async (payload) => {
    if (!payload?.webhook)
...
exports.checkFezLockerAvailability = async (lockerId) => {
    if (!lockerId)
...
exports.deleteFezUser = async (userId) => {
    if (!userId)
...
exports.logoutFez = async (payload = {}) => {
