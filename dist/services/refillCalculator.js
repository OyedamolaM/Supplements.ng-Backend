...
exports.calculateRefill = void 0;
...
const buildUsageText = (dosageAmount, dosageUnit, frequencyPerDay, fallbackText = "") => {
    if (fallbackText)
        return fallbackText;
...
const calculateRefill = (orderItem, order) => {
    ...
    const pauseDelta = orderItem.isPaused && pausedAt ? Math.max(0, diffInDays(new Date(), pausedAt)) : 0;
    ...
    if (manualDaysSupply) {
        ...
        const dueDate = new Date(new Date(purchaseDate).getTime() + (effectiveDaysSupply + totalPausedDays) * MS_PER_DAY);
    ...
    const dueDate = new Date(new Date(purchaseDate).getTime() + (daysSupply + totalPausedDays) * MS_PER_DAY);
    const daysLeft = diffInDays(dueDate, new Date());
    const status = daysLeft < 0
        ? "refill_overdue"
        : daysLeft === 0
...
exports.calculateRefill = calculateRefill;
