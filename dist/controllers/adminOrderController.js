...
exports.createOrderForUser = async (req, res) => {
    try {
        ...
        let inventoryChecks = [];
        if (!branch.isOnline) {
            inventoryChecks = await Promise.all(orderProducts.map(async (item) => {
            ...
            const outOfStock = inventoryChecks.find(({ item, inventory }) => !inventory || inventory.quantity < item.quantity);
            if (outOfStock) {
        ...
        if (!branch.isOnline) {
            await Promise.all(inventoryChecks.map(({ item, inventory }) => BranchInventory.updateOne({ _id: inventory._id }, { $inc: { quantity: -item.quantity } })));
            await Promise.all(orderProducts.map((item) => InventoryMovement.create({
                branch: resolvedBranchId,
                product: item.product,
                type: "sale",
...
exports.claimOnlineOrder = async (req, res) => {
    try {
        ...
        const inventoryChecks = await Promise.all(order.products.map(async (item) => {
            const inventory = await BranchInventory.findOne({
