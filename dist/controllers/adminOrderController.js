...
const { prisma, newId, fromDbUserRole, toLegacyOrder, legacyOrderStatusToDb, } = require("../utils/prismaLegacy");
const { generateReceipt } = require("../utils/receiptGenerator");
const { sendBrevoEmail, buildOrderConfirmationEmail, buildOrderStatusEmail, } = require("../services/emailService");
...
const normalizeUsageMode = (value) => (value || "FIXED").toString().trim().toUpperCase() === "AS_NEEDED"
    ? "AS_NEEDED"
    : "FIXED";
...
exports.createOrderForUser = async (req, res) => {
    try {
        ...
        if (branch.isOnline) {
            const hasShipping = REQUIRED_SHIPPING_FIELDS.every((field) => shippingAddress && shippingAddress[field]);
            if (!hasShipping) {
...
exports.dispatchFezOrder = async (req, res) => {
    try {
        ...
        const lockerId = (order.deliveryMeta && order.deliveryMeta.lockerId) ||
            (order.deliveryMeta && order.deliveryMeta.lockerID) ||
            null;
            ...
            data: {
                ...
                deliveryMeta: {
                    ...
                    sender: req.body?.thirdparty
                        ...
                        : undefined,
                    dispatchHistory: [
                        ...((order.deliveryMeta || {}).dispatchHistory || []),
                        {
...
exports.updateOrderStatus = async (req, res) => {
    try {
        ...
        const nextDeliveryMeta = nextEta !== undefined
            ? {
...
exports.updateOrderItemDosage = async (req, res) => {
    try {
        ...
        const nextDosageAmount = normalizeNumber(req.body?.dosageAmount, orderItem.recommendedDosageAmountSnapshot || 0);
        const nextFrequency = normalizeNumber(req.body?.frequencyPerDay, orderItem.recommendedFrequencyPerDaySnapshot || 0);
        const nextDosageUnit = normalizeText(req.body?.dosageUnit, orderItem.recommendedDosageUnitSnapshot || "");
        const nextUsageText = normalizeText(req.body?.usageText, orderItem.recommendedUsageTextSnapshot || "");
        const nextUsageMode = normalizeUsageMode(req.body?.usageMode || orderItem.usageModeSnapshot);
