...
const { sendBrevoEmail, buildVerificationEmail, buildPasswordResetEmail, buildWelcomeEmail, } = require("../services/emailService");
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        const expired = user.passwordResetExpiresAt &&
        ...
        if (!user) {
            if (!email) {
                return res.status(400).json({
                    message: "Apple sign in did not return an email. Please use email login once to link your Apple account.",
                });
            }
            const randomPassword = await bcrypt.hash(newId(), 10);
            user = await prisma.user.create({
            ...
            const updates = {};
            if (appleSubject && !user.appleSubject)
                updates.appleSubject = appleSubject;
            if (!user.emailVerified) {
                updates.emailVerified = true;
                updates.emailVerifiedAt = new Date();
            }
