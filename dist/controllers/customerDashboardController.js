...
const toDashboardReminder = (record) => ({
    _id: record.id,
    id: record.id,
...
const fetchDashboardReminders = async (userId) => {
...
exports.syncReminders = async (req, res) => {
    try {
        ...
        const reminderByProductId = new Map(existingReminders.map((item) => [item.productId, item]));
        ...
        for (const order of orders) {
            for (const item of order.items || []) {
                if (!item.productId || processed.has(item.productId))
                    continue;
...
exports.getNotifications = async (req, res) => {
    try {
        ...
        if (req.user?.phone) {
            const dueToday = refillNotifications.filter((item) => item.status === "refill_due_today" && !readComputed.has(item.computedKey));
