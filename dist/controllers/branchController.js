...
exports.updateBranchInventory = async (req, res) => {
    try {
        ...
        if (threshold > 0) {
            ...
            const currentMap = new Map(currentInventory.map((item) => [item.product.toString(), item.quantity]));
        ...
        for (const item of items) {
            ...
            const updated = await BranchInventory.findOneAndUpdate({ branch: req.params.id, product: item.productId }, { $set: { quantity: nextQty } }, { upsert: true, new: true });
