...
const FEZ_WEBHOOK_SECRET_HEADER = (process.env.FEZ_WEBHOOK_SECRET_HEADER || "x-fez-secret")
    ...
    .trim()
    .toLowerCase();
const readHeader = (req, name) => req.headers?.[name] || req.headers?.[name.toLowerCase()];
...
const extractOrderIdentifiers = (payload) => {
    const uniqueId = pickFirst(payload?.uniqueID, payload?.uniqueId, payload?.orderId, payload?.order_id, payload?.data?.uniqueID, payload?.data?.uniqueId, payload?.data?.orderId, payload?.data?.order_id);
    const orderNo = pickFirst(payload?.orderNo, payload?.orderNumber, payload?.order_number, payload?.data?.orderNo, payload?.data?.orderNumber, payload?.data?.order_number);
    return { uniqueId, orderNo };
};
const extractTrackingUrl = (payload) => pickFirst(payload?.trackingUrl, payload?.trackingURL, payload?.tracking_url, payload?.data?.trackingUrl);
const extractStatus = (payload) => pickFirst(payload?.status, payload?.deliveryStatus, payload?.orderStatus, payload?.currentStatus, payload?.state, payload?.data?.status, payload?.data?.deliveryStatus, payload?.data?.orderStatus);
