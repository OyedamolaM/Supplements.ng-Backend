...
const getSenderByKey = (key = "") => {
  ...
  if (!normalized) return getSender();
...
const buildOrderConfirmationEmail = ({
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #1f2937;">
      <p>Hi ${safeName},</p>
      <p>Your order <strong>${escapeHtml(orderId)}</strong> has been placed on ${escapeHtml(
    formattedDate
  )}.</p>
      ...
      <p style="margin:8px 0;"><strong>Payment:</strong> ${escapeHtml(
        paymentMethod || "Paystack"
      )}</p>
      ${
        addressLine
          ? `<p style="margin:8px 0;"><strong>Shipping:</strong> ${escapeHtml(
              addressLine
            )}</p>`
          : ""
      }
      ${
        viewUrl
          ? `<p style="margin:8px 0;"><a href="${escapeHtml(
              viewUrl
            )}" target="_blank" rel="noreferrer">View your order</a></p>`
          : ""
      }
...
const buildReceiptEmail = ({
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #1f2937;">
      <p>Hi ${safeName},</p>
      <p>Your payment was successful. Attached is your receipt for order <strong>${escapeHtml(
        orderId
      )}</strong>.</p>
      ...
      ${
        viewUrl
          ? `<p><a href="${escapeHtml(
              viewUrl
            )}" target="_blank" rel="noreferrer">View order</a></p>`
          : ""
      }
      <p style="color:#6b7280;">Thank you for shopping with supplements.ng.</p>
    </div>
...
const buildWelcomeEmail = ({ name }: { name: string }) => {
  const safeName = escapeHtml(name || "there");
