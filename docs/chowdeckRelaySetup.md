...
1. Call `POST /api/orders/shipping-quote` for a local home delivery.
2. Store the returned `deliveryFeeId`, `shippingEta`, and `data.totalCost`.
3. Send `deliveryFeeId`, `shippingQuote`, and the backend-calculated shipping fee into `POST /api/orders`.
